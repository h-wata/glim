...
Eigen::Vector4d get_vec4(const void * x, const void * y, const void * z)
{
  return Eigen::Vector4d(
    *reinterpret_cast<const T *>(x), *reinterpret_cast<const T *>(y), *reinterpret_cast<const T *>(z), 1.0);
}

static RawPoints::Ptr extract_raw_points(
  const PointCloud2 & points_msg, const std::string & intensity_channel, const std::string & ring_channel)
{
  ...
  if (x_type == PointField::FLOAT32 && y_offset == x_offset + sizeof(float) && z_offset == y_offset + sizeof(float)) {
    ...
    for (int i = 0; i < num_points; i++) {
      ...
      raw_points->points[i] << Eigen::Map<const Eigen::Vector3f>(reinterpret_cast<const float *>(x_ptr)).cast<double>(),
        1.0;
    }
  } else if (
    x_type == PointField::FLOAT64 && y_offset == x_offset + sizeof(double) && z_offset == y_offset + sizeof(double)) {
  ...
  if (color_offset >= 0) {
    ...
    if (color_type == PointField::FLOAT32) {
      ...
      for (int i = 0; i < num_points; i++) {
        const auto * color_ptr = &points_msg.data[points_msg.point_step * i + color_offset];
        raw_points->colors[i] =
          Eigen::Matrix<unsigned char, 4, 1>(reinterpret_cast<const std::uint8_t *>(color_ptr)).cast<double>() / 255.0;
      }
    } else {
...
static RawPoints::Ptr extract_raw_points(
  const PointCloud2 & points_msg, const std::string & intensity_channel = "intensity")
{
  return extract_raw_points(points_msg, intensity_channel, "");
}

static RawPoints::Ptr extract_raw_points(
  const PointCloud2ConstPtr & points_msg, const std::string & intensity_channel = "intensity")
{
  return extract_raw_points(*points_msg, intensity_channel, "");
}

static PointCloud2ConstPtr frame_to_pointcloud2(
  const std::string & frame_id, const double stamp, const gtsam_points::PointCloud & frame)
{
  ...
  const Eigen::Vector4f * colors =
    frame.aux_attributes.count("colors") ? frame.aux_attribute<Eigen::Vector4f>("colors") : nullptr;
  if (colors) {
  ...
  for (int i = 0; i < frame.size(); i++) {
    ...
    if (colors) {
      const Eigen::Matrix<std::uint8_t, 4, 1> rgba =
        (colors[i].array() * 255.0f).min(255.0f).max(0.0f).cast<std::uint8_t>();
